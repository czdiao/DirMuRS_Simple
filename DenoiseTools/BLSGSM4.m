function [w1] = BLSGSM4(coef,C_w,logz,params)

% This code implements the decimated GSM with p(z) Gaussian Hermite
% density. NOT RIGHT so far.
% Input:
...
ske = skewness(norpatch(:));
kur = kurtosis(norpatch(:));
...
%% How to estimation of z ?! This is a problem!!
% mu and sig2 in the pdf is not mu_z = 1, or sig2_z estimated above
...
% a = linspace(-10,10,size(zn,1));
% polyn_a = 1+ske/6*hermite(a/sig2_z,3)+(kur-3)/24*hermite(a/sig2_z,4);
% polynder_a = ske/(2*sig2_z)*hermite(a/sig2_z,2)+(kur-3)/(6*sig2_z)*hermite(a/sig2_z,3);
% f_cost_a = -a/sig2_z^2 + polynder_a./polyn_a + sum((a_vec-zn)./(a_vec+lamb_vec_i).^2);

% zzp_a = normpdf(a,1,sig2_z).*polyn_a;
% logn_a = lognpdf(a,mu_l,sig2_l);
...
%     polyn_b = 1+ske/6*hermite(b/sig2_z,3)+(kur-3)/24*hermite(b/sig2_z,4);
%     polynder_b = ske/(2*sig2_z)*hermite(b/sig2_z,2)+(kur-3)/(6*sig2_z)*hermite(b/sig2_z,3);
%     f_cost_b = -b/sig2_z^2 + polynder_b./polyn_b + sum((b_vec-zn)./(b_vec+lamb_vec_i).^2); 
